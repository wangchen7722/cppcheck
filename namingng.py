...
def check_class_naming(conf,cfg):
    ...
    for fnc in cfg.functions:
        if fnc.type not in ('Constructor','Destructor'):
            continue
        if fnc.name in checked_classes:
            continue
        checked_classes.add(fnc.name)
    ...
    for token in cfg.tokenlist:
        if token.str != 'class' and token.str != 'struct':
            continue
        next_token = token.next
        while next_token and (next_token.str.isspace() or next_token.str in ('final', ':')):
            next_token = next_token.next
        if next_token and next_token.str and next_token.str not in checked_classes:
            checked_classes.add(next_token.str)
            mockToken = DataStruct(token.file, token.linenr, next_token.str, next_token.column)
            msgType = 'Class'
            for exp in conf.class_name:
                evalExpr(conf.class_name, exp, mockToken, msgType)
